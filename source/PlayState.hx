...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('backgrounds/$curStage/halloween_bg', -200, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('backgrounds/$curStage/halloween_bg_low', -200, -100);
				}
				add(halloweenBG);
			...
			case 'philly': //Week 3
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('backgrounds/$curStage/sky', -100, 0, 0.1, 0.1);
					add(bg);
				}

				var city:BGSprite = new BGSprite('backgrounds/$curStage/city', -10, 0, 0.3, 0.3);
				...
				for (i in 0...5)
				{
					var light:BGSprite = new BGSprite('backgrounds/$curStage/win' + i, city.x, city.y, 0.3, 0.3);
					light.visible = false;
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('backgrounds/$curStage/behindTrain', -40, 50);
					add(streetBehind);
				}

				phillyTrain = new BGSprite('backgrounds/$curStage/train', 2000, 360);
				add(phillyTrain);
				...
				var street:BGSprite = new BGSprite('backgrounds/$curStage/street', -40, 50);
				add(street);

			case 'limo': //Week 4
				var skyBG:BGSprite = new BGSprite('backgrounds/$curStage/limoSunset', -120, -50, 0.1, 0.1);
				add(skyBG);

				if(!ClientPrefs.lowQuality) {
					limoMetalPole = new BGSprite('backgrounds/$curStage/gore/metalPole', -500, 220, 0.4, 0.4);
					add(limoMetalPole);

					bgLimo = new BGSprite('backgrounds/$curStage/bgLimo', -150, 480, 0.4, 0.4, ['background limo pink'], true);
					add(bgLimo);

					limoCorpse = new BGSprite('backgrounds/$curStage/gore/noooooo', -500, limoMetalPole.y - 130, 0.4, 0.4, ['Henchmen on rail'], true);
					add(limoCorpse);

					limoCorpseTwo = new BGSprite('backgrounds/$curStage/gore/noooooo', -500, limoMetalPole.y, 0.4, 0.4, ['henchmen death'], true);
					add(limoCorpseTwo);
					...
					limoLight = new BGSprite('backgrounds/$curStage/gore/coldHeartKiller', limoMetalPole.x - 180, limoMetalPole.y - 80, 0.4, 0.4);
					add(limoLight);
					...
					//PRECACHE BLOOD
					var particle:BGSprite = new BGSprite('backgrounds/$curStage/gore/stupidBlood', -400, -400, 0.4, 0.4, ['blood'], false);
					particle.alpha = 0.01;
				...
				limo = new BGSprite('backgrounds/$curStage/limoDrive', -120, 550, 1, 1, ['Limo stage'], true);

				fastCar = new BGSprite('backgrounds/$curStage/fastCarLol', -300, 160);
				fastCar.active = true;
				limoKillingState = 0;

			case 'mall': //Week 5 - Cocoa, Eggnog
				var bg:BGSprite = new BGSprite('backgrounds/$curStage/bgWalls', -1000, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				if(!ClientPrefs.lowQuality) {
					upperBoppers = new BGSprite('backgrounds/$curStage/upperBop', -240, -90, 0.33, 0.33, ['Upper Crowd Bob']);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					upperBoppers.updateHitbox();
					add(upperBoppers);

					var bgEscalator:BGSprite = new BGSprite('backgrounds/$curStage/bgEscalator', -1100, -600, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);
				}

				var tree:BGSprite = new BGSprite('backgrounds/$curStage/christmasTree', 370, -250, 0.40, 0.40);
				add(tree);

				bottomBoppers = new BGSprite('backgrounds/$curStage/bottomBop', -300, 140, 0.9, 0.9, ['Bottom Level Boppers Idle']);
				bottomBoppers.animation.addByPrefix('hey', 'Bottom Level Boppers HEY', 24, false);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
				bottomBoppers.updateHitbox();
				add(bottomBoppers);

				var fgSnow:BGSprite = new BGSprite('backgrounds/$curStage/fgSnow', -600, 700);
				add(fgSnow);

				santa = new BGSprite('backgrounds/$curStage/santa', -840, 150, 1, 1, ['santa idle in fear']);
				add(santa);
				CoolUtil.precacheSound('Lights_Shut_off');

			case 'mallEvil': //Week 5 - Winter Horrorland
				var bg:BGSprite = new BGSprite('backgrounds/$curStage/evilBG', -400, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var evilTree:BGSprite = new BGSprite('backgrounds/$curStage/evilTree', 300, -300, 0.2, 0.2);
				add(evilTree);

				var evilSnow:BGSprite = new BGSprite('backgrounds/$curStage/evilSnow', -200, 700);
				add(evilSnow);
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				var bgSky:BGSprite = new BGSprite('backgrounds/$curStage/weebSky', 0, 0, 0.1, 0.1);
				add(bgSky);
				bgSky.antialiasing = false;

				var repositionShit = -200;

				var bgSchool:BGSprite = new BGSprite('backgrounds/$curStage/weebSchool', repositionShit, 0, 0.6, 0.90);
				add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('backgrounds/$curStage/weebStreet', repositionShit, 0, 0.95, 0.95);
				add(bgStreet);
				bgStreet.antialiasing = false;

				var widShit = Std.int(bgSky.width * 6);
				if(!ClientPrefs.lowQuality) {
					var fgTrees:BGSprite = new BGSprite('backgrounds/$curStage/weebTreesBack', repositionShit + 170, 130, 0.9, 0.9);
				...
				if(!ClientPrefs.lowQuality) {
					var treeLeaves:BGSprite = new BGSprite('backgrounds/$curStage/petals', repositionShit, -40, 0.85, 0.85, ['PETALS ALL'], true);
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				var posY = 200;
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('backgrounds/$curStage/animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 2'], true);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);

					bgGhouls = new BGSprite('backgrounds/$curStage/bgGhouls', -100, 190, 0.9, 0.9, ['BG freaks glitch instance'], false);
					...
					var bg:BGSprite = new BGSprite('backgrounds/$curStage/animatedEvilSchool_low', posX, posY, 0.8, 0.9);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);
				}
		...
		#if (MODS_ALLOWED && LUA_ALLOWED)
		var doPush:Bool = false;
		var luaFile:String = 'songs/' + Paths.formatToSongPath(SONG.song) + '/script.lua';
		if(FileSystem.exists(Paths.modFolders(luaFile))) {
			luaFile = Paths.modFolders(luaFile);
			doPush = true;
		} else {
	...
	function updateScore() {
		...
		+ divider + 'Accuracy: ' + (ratingString == 'N/A' ? 0 : Math.floor(ratingPercent * 100)) + '%'
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image('ui/' + uiElement + daRating + uiPostfix));
		rating.screenCenter();
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image('ui/' + uiElement + 'combo' + uiPostfix));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('ui/' + uiElement + 'combo/num' + Std.int(i) + uiPostfix));
			...
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					case 1 | 2 | 3 | 4 | 5 | 6 | 7:
						...
						storyPlaylist.length <= 1 && WeekData.getWeekFileName() == ('week' + arrayIDs[i]) && !changedDifficulty) {
