...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 435).loadGraphic(Paths.image('menus/story/tracks'));
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/story/backgrounds/' + assetName)) || #end
			Assets.exists(Paths.image('menus/story/backgrounds/' + assetName), IMAGE)) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/story/weeks/' + assetName)) || #end
			Assets.exists(Paths.image('menus/story/weeks/' + assetName), IMAGE)) {
