...
/**
	*DEBUG MODE
 */
class NewCharacterEditorState extends MusicBeatState
{
	var char:Character;
	...
	var displayCharacter:FlxSprite;

	var tipHelp:Bool = false;
	var tipText:FlxText;
	...
	override function create()
	{
		...
		displayCharacter = new FlxSprite().loadGraphic(Paths.image('menus/editors/character/dad'));
		displayCharacter.alpha = 0.8;
		...
		tipText = new FlxText(FlxG.width - 20, FlxG.height, 0, "PRESS F1 FOR HELP", 12);
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:BGSprite = new BGSprite('backgrounds/school/weebSky', OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference, 0.1, 0.1);
			bgLayer.add(bgSky);
			bgSky.antialiasing = false;

			var repositionShit = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BGSprite = new BGSprite('backgrounds/school/weebSchool', repositionShit, -playerYDifference + 6, 0.6, 0.90);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BGSprite = new BGSprite('backgrounds/school/weebStreet', repositionShit, -playerYDifference, 0.95, 0.95);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == healthIconInputText) {
				leHealthIcon.changeIcon(healthIconInputText.text, false, true);
	...
	function genBoyOffsets():Void
	{
		...
		if(animsTxtGroup.length < 1) {
			var text:FlxText = new FlxText(10, 55, 0, "ERROR! No animations found.", 15);
			text.scrollFactor.set();
			text.borderSize = 1.5;
			animsTxtGroup.add(text);
	...
	override function update(elapsed:Float)
	{
		...
		if (tipHelp) {
			tipText.text = "E/Q - Camera Zoom In/Out
			\nJKLI - Move Camera
		...
		if (FlxG.keys.justPressed.F1) tipHelp = !tipHelp;

		camMenu.zoom = camHUD.zoom;
		ghostChar.setPosition(char.x, char.y);
		if (char.isPlayer) {
			displayCharacter.setPosition(OFFSET_X + 100,350);
			displayCharacter.loadGraphic(Paths.image('menus/editors/character/bf'));
		} else {
			displayCharacter.setPosition(OFFSET_X + 100, 0);
			displayCharacter.loadGraphic(Paths.image('menus/editors/character/dad'));
		}
		super.update(elapsed);
	}
